...
class Form:
        ...
        if props_ids is not None:
            for prop_id in props_ids:
                if self.run_gremlin_query("g.E('" + prop_id + "').properties('help text')"):
                    help_text += (
                        self.run_gremlin_query("g.E('" + prop_id + "').properties('text').value()")[0]
                        + ": "
                        + self.run_gremlin_query("g.E('" + prop_id + "').properties('help text').value()")[0]
                        + "\n"
                    )
        return help_text
    ...
    def add_open_question(self, node_id: str, modif_crypted: bool, previous_answer: Optional[list] = None) -> tuple:
        ...
        if previous_answer is not None:  # If it has to be auto-completed before
            answer = st.text_area(
                ...
                help=self.build_question_help_text(node_id),
            )
        else:
            answer = st.text_area(
                label=question, height=100, label_visibility="visible", help=self.build_question_help_text(node_id)
            )
    ...
    def add_qcm_question(self, node_id: str, modif_crypted: bool, previous_answer: Optional[list] = None) -> tuple:
        ...
        if previous_answer is not None:  # If it has to be auto-completed before
            ...
            answer = st.selectbox(
                ...
                help=self.build_question_help_text(node_id, props_ids),
            )
        else:
            answer = st.selectbox(
                label=question, options=options, index=0, help=self.build_question_help_text(node_id, props_ids)
            )
        # If no answer given, we return None
    ...
    def add_qrm_question(self, node_id: str, modif_crypted: bool, previous_answer: Optional[list] = None) -> tuple:
        ...
        if previous_answer is not None:  # If it has to be auto-completed before
            ...
            answers = st.multiselect(
                label=question, options=options, default=None, help=self.build_question_help_text(node_id, props_ids)
            )
        # If no answer given, we return None
        if answers == []:
            return None, None, modif_crypted
        ...
        for list_answer in answers:
            for dict_answer in list_answer:
                ...
                if not new_node_id_exist:
                    self.run_gremlin_query(
                        "g.addV('Answer').property('partitionKey', 'Answer').property('id', '"
                        + new_node_id
                        + "').property('question', '"
                        + actual_node["properties"]["text"][0]["value"]
                ...
                if not next_new_node_id_exist:
                    if next_question_node["label"] == "end":
                        self.run_gremlin_query(
                            'g.addV("end").property("partitionKey", "Answer").property("id", "'
                            + next_new_node_id
                            + '")'
                        )
                    else:
                        self.run_gremlin_query(
                            ...
                            + next_new_node_id
                            + "').property('question', '"
                            + next_question_node["properties"]["text"][0]["value"]
                            + "').property('question_id', '"
    ...
    def display_bar_graph(self, edges_selected: dict) -> None:
        ...
        for key, value in edges_selected.items():
            ...
            if text:
                ...
                edges_selected_with_text[key] = {
                    "text": self.run_gremlin_query("g.E('" + key + "').label()")[0],
                    "nb_selected": value,
                }
    ...
    def display_bar_graph_v2(self, edge_selected) -> None:
        ...
        for key in edge_selected.keys():
            hover_text.append(
                "Q"
                + self.run_gremlin_query("g.E('" + key + "').outV().id()")[0]
                + " to Q"
                + self.run_gremlin_query("g.E('" + key + "').inV().id()")[0]
            )
        ...
        fig = go.Figure(
            data=[go.Bar(x=list(edge_selected.keys()), y=list(edge_selected.values()), hovertext=text, text=hover_text)]
        )
        fig.update_layout(
            title="Number of times each edge was selected",
